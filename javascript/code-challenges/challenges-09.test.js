...
const maxInArray = (arr) => {
  let lastNum = -99999999999;
  arr.reduce((acc, curr) => {
    lastNum = curr > lastNum ? curr : lastNum;
  });
  return lastNum;
};
...
const checkValues = (obj, value) => {
  return obj.class === value ? true : false;
};
...
const hasChildrenValues = (arr, character) => {
  for (let char of arr) {
    if (char.name === character && char.children) {
      return Object.values(char.children).length > 0 ? true : false;
    }
  }
  return false;
};
